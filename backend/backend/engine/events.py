...
def _same_description_group(
    ...
    """
    Group events that are likely instances of the same recurring
    ...
    This prevents unrelated transactions from being combined.
    """
    ...
    for event in events:
        key = (
            ...
            (event.description or "").strip().lower(),
            (event.currency or "").strip().upper(),
        )
